...
import com.example.voicereminder.domain.models.MissedPolicy
...
class ReminderAlarmReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                // Determine grace window based on recurrence type
                ...
                FileLogger.log("ALARM: Next trigger = $nextTrigger")
                
                if (nextTrigger != null) {
                    updatedReminder = updatedReminder.copy(nextTriggerAt = nextTrigger)
                    scheduler.schedule(updatedReminder)
                    FileLogger.log("ALARM: Rescheduled to $nextTrigger")
                } else {
                    if (reminder.recurrenceType == RecurrenceType.NONE) {
                        updatedReminder = updatedReminder.copy(isCompleted = true, completedAt = now)
                        FileLogger.log("ALARM: Marked as completed (no recurrence)")
                    }
                }
    ...
    private fun getGraceWindow(recurrenceType: RecurrenceType, recurrenceJson: String?): Long {
        ...
        if (recurrenceType == RecurrenceType.CUSTOM) {
            ...
            if (model is com.example.voicereminder.domain.models.RecurrenceModel.Custom) {
