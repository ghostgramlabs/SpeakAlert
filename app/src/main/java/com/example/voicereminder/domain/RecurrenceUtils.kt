...
object RecurrenceUtils {
    ...
    fun fromJson(type: RecurrenceType, json: String?): RecurrenceModel? {
        com.example.voicereminder.util.FileLogger.log("Recurrence: fromJson type=$type json=$json")
        if (json.isNullOrBlank()) {
             if (type == RecurrenceType.DAILY) return RecurrenceModel.Daily()
             com.example.voicereminder.util.FileLogger.log("Recurrence: JSON is blank for type=$type, returning null")
             return null
        }
        return try {
            val model = when (type) {
            ...
            if (model == null) {
                com.example.voicereminder.util.FileLogger.log("Recurrence: Parsed model is NULL for $json")
            }
            model
        } catch (e: Exception) {
            com.example.voicereminder.util.FileLogger.logError("Recurrence", "Json parse failed: $json", e)
            e.printStackTrace()
            null
        }
    }
    ...
    fun toJson(model: RecurrenceModel): String {
        ...
        com.example.voicereminder.util.FileLogger.log("Recurrence: toJson($model) -> $json")
        return json
    }
    ...
    fun computeNextTrigger(reminder: ReminderEntity, fromTime: Long = System.currentTimeMillis()): Long? {
        ...
        val model = fromJson(reminder.recurrenceType, reminder.recurrenceJson) ?: run {
            com.example.voicereminder.util.FileLogger.log("Recurrence: computeNextTrigger failed - invalid JSON for ${reminder.recurrenceType}")
            // Fallback for Daily if JSON missing
            if (reminder.recurrenceType == RecurrenceType.DAILY) RecurrenceModel.Daily() else null
        } ?: return null

        // Check End Rules
        if (isEnded(model, reminder)) {
             com.example.voicereminder.util.FileLogger.log("Recurrence: Recurrence ENDED via rule: ${model.endRule}")
             return null
        }
        ...
        com.example.voicereminder.util.FileLogger.log("Recurrence: Computed raw next=$nextTrigger for $model")
        ...
        var attempts = 0
        while (nextTrigger <= fromTime && attempts < 50) {
             com.example.voicereminder.util.FileLogger.log("Recurrence: Result $nextTrigger <= $fromTime. Advancing again...")
        ...
        if (nextTrigger <= fromTime) {
             com.example.voicereminder.util.FileLogger.log("Recurrence: FAILED to find future time after 50 attempts. Defaulting to +1 min to avoid crash.")
    ...
    private fun isEnded(model: RecurrenceModel, reminder: ReminderEntity): Boolean {
        ...
        return when (end.type) {
            ...
            EndRuleType.UNTIL_DATE -> System.currentTimeMillis() > (end.endDateMillis ?: Long.MAX_VALUE)
    ...
    private fun computeNextCustom(baseTrigger: Long, rule: RecurrenceModel.Custom, fromTime: Long): Long {
        com.example.voicereminder.util.FileLogger.log("Recurrence: Custom calc. Base=${java.util.Date(baseTrigger)}, From=${java.util.Date(fromTime)}, Rule=$rule")
        ...
        if (rule.unit == TimeUnit.MONTHS || rule.unit == TimeUnit.DAYS || rule.unit == TimeUnit.WEEKS) {
            ...
            com.example.voicereminder.util.FileLogger.log("Recurrence: Custom (Cal) result=${java.util.Date(cal.timeInMillis)}")
            ...
            // Safety check
            if (intervalMillis <= 0) {
                 com.example.voicereminder.util.FileLogger.log("Recurrence: Invalid interval millis $intervalMillis")
                 return fromTime + 60000 // Failsafe 1 min
            }
            
            var nextTime = baseTrigger
            if (nextTime > fromTime) {
                com.example.voicereminder.util.FileLogger.log("Recurrence: Base is future, keeping ${java.util.Date(nextTime)}")
                return nextTime
            }
            ...
            com.example.voicereminder.util.FileLogger.log("Recurrence: Custom (Millis) result=${java.util.Date(nextTime)}")
            return nextTime
        }
    }
